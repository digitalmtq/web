exports.handler = async (event) => {
  try {
    ...
    if (!res.ok) return { statusCode: res.status, body: JSON.stringify({ error: "Gagal ambil data GitHub" }) };

    const fileData = await res.json();
    const sha = fileData.sha;
    let santri = JSON.parse(Buffer.from(fileData.content, "base64").toString("utf-8"));
    ...
    const updateRes = await fetch(url, {
      ...
      body: JSON.stringify({ message: `Hapus santri id ${id}`, content: Buffer.from(JSON.stringify(newSantri, null,2)).toString("base64"), sha })
    });
